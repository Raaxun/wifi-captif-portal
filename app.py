...
def matrix():
    if not session.get('authenticated'):
        return redirect(url_for('login'))
    users = get_users_except(session['login'])
    return render_template('matrix.html', username=session['login'], users=users, receiver=None, messages=[])

@app.route('/matrix/<receiver>', methods=['GET', 'POST'])
def matrix_chat(receiver):
    if not session.get('authenticated'):
        return redirect(url_for('login'))
    sender_id = get_user_id(session['login'])
    ...
    if request.method == 'POST':
        ...
        if content:
            ...
            return redirect(url_for('matrix_chat', receiver=receiver))

    messages = get_messages(sender_id, receiver_id)
    users = get_users_except(session['login'])
    return render_template('matrix.html', username=session['login'], receiver=receiver, messages=messages, users=users)
